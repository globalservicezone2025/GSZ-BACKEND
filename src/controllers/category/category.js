...
export const updateCategory = async (req, res) => {
  try {
    return await prisma.$transaction(async (tx) => {
      const { userId, name, serial } = req.body; // Include serial
          ...
          async (error, result) => {
            ...
            const category = await prisma.category.update({
              ...
              data: {
                ...
                name,
                image: result.secure_url,
                serial: parseInt(serial) || findCategory.serial, // Parse serial to integer
...
export const countActiveCategories = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(jsonResponse(true, "Total active categories count", { count }));
  } catch (error) {
    console.log(error);
    return res.status(500).json(jsonResponse(false, error, null));
  }
